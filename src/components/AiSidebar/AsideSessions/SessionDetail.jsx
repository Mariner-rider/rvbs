...
import Sidebar from "../AsideToggle/MainSideBar";
...
useEffect(() => {
  const fetchSessionData = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/session-details/${id}/`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${userToken}`,
        },
      });
      const chatData = response.data.data.chat || response.data.data; // Flexible
                        ...
                        {item.sender === "user" && (
                          <div className="col-start-1 col-end-8 p-1 lg:p-3 rounded-lg">
                            <div className="flex flex-row items-center">
                              ...
                              <div className="relative ml-3 text-md bg-white border border-gray-400 border-opacity-40 dark:bg-gray-900/50 py-3 px-4 rounded-xl">
                                <div className="text-gray-800 dark:text-gray-200 font-bold tracking-wide">
                                  {item.content}
                                </div>
                              </div>
                            </div>
                        ...
                        {item.sender === "bot" && (
                          <div className="col-start-1 col-end-8 p-1 lg:p-3 rounded-lg">
                            <div className="flex flex-row items-start">
                              <div className="flex items-center justify-center w-6 h-10 lg:w-10 rounded-full flex-shrink-0">
                                <img className="w-8" src={AiLogo} />
                              </div>
                              <div className="flex justify-between items-start relative ml-3 text-gray-800 text-md w-full bg-white border border-gray-400 border-opacity-40 dark:bg-gray-900/50 p-4 rounded-xl">
                                <div
                                  className="text-gray-800  p-2 md:p-3 dark:text-gray-200 tracking-wide "
